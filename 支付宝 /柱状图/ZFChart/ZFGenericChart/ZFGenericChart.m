...
- (void)setXLineTitleLabel{
    for (NSInteger i = 0; i < self.xLineTitleArray.count; i++) {
        CGFloat xPos = self.axisLine.xLineStartXPos + XLineItemGapLength + (XLineItemWidth + XLineItemGapLength+60) * i;
        CGFloat yPos = self.axisLine.xLineStartYPos + 5;
        CGFloat width = XLineItemWidth;
        CGFloat height = _xLineLabelHeight;
        
        //label的中心点
        CGPoint label_center = CGPointMake(xPos + width * 0.5 - 40, yPos + height * 0.5-280);
