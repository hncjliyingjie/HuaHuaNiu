...
@property (nonatomic, strong) UILabel * personLabel2;
@property (nonatomic, strong) UILabel * personLabel3;
...
- (void)drawBar{
    ...
    for (NSInteger i = 0; i < self.xLineValueArray.count; i++) {
        ...
        CGFloat xPos = self.genericChart.axisStartXPos + XLineItemGapLength + (XLineItemWidth + XLineItemGapLength + 60) * i;
//        CGFloat yPos = self.genericChart.yLineMaxValueYPos;
        ...
        //-200
        ZFBar * bar = [[ZFBar alloc] initWithFrame:CGRectMake(xPos-40, -200, width, height)];
        
        NSLog(@"%f",height);

        NSLog(@"%f",[self.xLineValueArray[i] floatValue] / _yLineMaxValue);
        bar.percent = [self.xLineValueArray[i] floatValue] / _yLineMaxValue;

        bar.isShadow = NO;//_isShadow;
        [bar strokePath];
 ...
 *  显示bar上的label
 */
#pragma mark - 显示bar上的label
- (void)showLabelOnChart{
    for (NSInteger i = 0; i < self.barArray.count; i++) {
        ZFBar * bar = self.barArray[i];
        //label的中心点
        CGPoint label_center = CGPointMake(bar.center.x, bar.endYPos + self.genericChart.yLineEndYPos-270);
        ...
        ZFLabel * label = [[ZFLabel alloc] initWithFrame:CGRectMake(0, 0, 60, rect.size.height)];
//        if (i == 0) {
//            self.personLabel1 = label;
//        }else if(i == 1){
//            self.personLabel2 = label;
//        }else{
//            self.personLabel3 = label;
//        }
//        NSString *str=[NSString stringWithFormat:@"%i",[[self.xLineValueArray objectAtIndex:i] intValue]*10];
//        NSArray *array=@[@"100",@"3333",@"2222"];
        label.text = [_xLineValueArray objectAtIndex:i];
        NSLog(@"%@",[NSString stringWithFormat:@"%i",[[self.xLineValueArray objectAtIndex:i] intValue]*10]);
        NSLog(@"%i",[[self.xLineValueArray objectAtIndex:i] intValue]*10);
