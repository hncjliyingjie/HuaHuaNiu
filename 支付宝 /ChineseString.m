...
#pragma mark - 返回排序后的模型
+(NSMutableArray*)LetterSortModelArray:(NSArray*)stringArr
{
    NSMutableArray *tempArray = [self ReturnSortChineseModelArrar:stringArr];
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((ChineseString*)object).pinYin substringToIndex:1];
        NSString *string = ((ChineseString*)object).string;
        NSString *layer = ((ChineseString*)object).layer;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseModelArrar:(NSArray*)modelArr
{
    ...
    {
        ChineseString *chineseString=[[ChineseString alloc]init];
        YHCityChouseModel * model = [modelArr objectAtIndex:i];
        chineseString.string= model.region_name;
        chineseString.layer = model.layer;
        if(chineseString.string==nil){
            chineseString.string=@"";
        }
        //去除两端空格和回车
    ...
    for (int i = 0; i < chineseStringsArray.count; i++) {
        ChineseString * ste = chineseStringsArray[i];
        NSLog(@"%@-------%@",ste.string,ste.layer);
    }
    return chineseStringsArray;
}
