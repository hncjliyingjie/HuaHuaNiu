...
//    width = self.frame.size.height - (2 * kVerticalBuffer);
    width=(self.frame.size.width-4*kVerticalBuffer)/3.5;
...
//        btn.frame = CGRectMake(leftLength, kVerticalBuffer, 60, 60);
...
//                       forState:UIControlStateNormal];
        [btn setBackgroundImage:[UIImage imageNamed:@"1.png"]
                       forState:UIControlStateNormal];
...
//             forState:UIControlStateNormal];
        ...
        UILabel *lbl=[[UILabel alloc]initWithFrame:CGRectMake(leftLength, kVerticalBuffer+width, 60, 30)];
        lbl.text=[NSString stringWithFormat:@"报刊广告%d",i];
        lbl.font=[UIFont systemFontOfSize:13];
        lbl.textAlignment=NSTextAlignmentCenter;
        [self addSubview:lbl];

        leftLength += width + kHorizontalBuffer;
//        leftLength += 60 + kHorizontalBuffer;
        
    }

//    self.contentSize = CGSizeMake(leftLength, self.frame.size.height);
     self.contentSize = CGSizeMake(leftLength, 0);
}
