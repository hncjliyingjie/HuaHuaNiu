...
-(void)setColumns:(NSInteger)columns{
    ...
    for(NSInteger i = 1 ; i<=self.dataArr.count; i++){
        ...
        //左右间隙
        CGFloat outLeftGap = 20.0f;
        //button左右之间的间隙
        CGFloat insideLeftGap = 5.0f;
        //顶部间隙
        CGFloat outTopGap =  35 + 5;
        //button上下之间的间隙
        CGFloat insideTopGap = 5;
        //按钮的长度
        CGFloat length  = (ScreenWidth - 2 * outLeftGap - insideLeftGap * (columns-1))/columns;
        // 按钮的高度
        CGFloat height = 30;
        //计算button的frame
        CGRect btnFrame = CGRectMake(outLeftGap + (rank-1) * (length + insideLeftGap), outTopGap + (row-1) * (height+ insideTopGap), length, height);
        
        ItemButton * button = [[ItemButton alloc]initWithFrame:btnFrame withTitle:[self.dataArr objectAtIndex:i-1]];
...
- (void)itemSelect:(ItemButton *)sender{
