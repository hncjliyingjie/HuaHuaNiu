//
//  HGSpecialViewController.m
//  视频模块
//
//  Created by Vking on 16/1/12.
...
#define HGFindCellName @"HGspeciCellName"
#define HGFindHeaderName @"HGspeciHeaderName"




@interface HGSpecialViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,HGWaterFlowDelegate,RefreshBaseViewDelegate>
...
@implementation HGSpecialViewController
...
-(void)sendRequest
{
    NSString *urlH;
    if (_count==1) {
        urlH=[NSString stringWithFormat:@"http://daiyancheng.cn/appvideo/video_my.do?token=test&member_id=%@&currentPage=1&showCount=%@",_member,_showCount];
        self.findCollectionView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);
    }
    else{
       urlH=[NSString stringWithFormat:@"http://daiyancheng.cn/appvideo/video_list.do?token=test&member_id=%@&currentPage=1&showCount=%@&ll=%@",_member,_showCount,[[NSUserDefaults standardUserDefaults] objectForKey:@"userlocatll"]];
    }
    [HGHttpTool getWirhUrl:urlH parms:nil success:^(id json) {
...
-(void)loadMoreData
{
    ...
    [HGHttpTool getWirhUrl:[NSString stringWithFormat:@"http://daiyancheng.cn/appvideo/video_list.do?token=test&member_id=%@&currentPage=%@&showCount=%@&ll=%@",_member,_currentPage,_showCount,[[NSUserDefaults standardUserDefaults] objectForKey:@"userlocatll"]] parms:nil success:^(id json) {
