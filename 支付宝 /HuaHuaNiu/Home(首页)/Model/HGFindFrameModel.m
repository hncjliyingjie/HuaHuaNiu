...
-(void)setFindModel:(HGFindModel *)findModel
{
    ...
    NSString *shopname=[self subStrFromFitScreen:findModel.title];
    ...
    CGSize priceSize=[findModel.store_name  sizeWithAttributes:@{NSFontAttributeName:HGfont(14)}];
    
    
    
    _priceF=CGRectMake(priceX, priceY,cellW-20 , (priceSize.width / (cellW - 20) +1) * priceSize.height);
    
    //4.设置商品来源的位置
    CGFloat addressX=VMargin;
    CGFloat addressY=CGRectGetMaxY(_priceF)+findLineMargin;
    
    NSString *addressStr=[NSString stringWithFormat:@"%@",findModel.add_time_x];
