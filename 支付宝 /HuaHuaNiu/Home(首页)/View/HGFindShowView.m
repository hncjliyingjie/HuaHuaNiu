...
{
    //1.设置图片
    NSString *strUrl =  [NSString stringWithFormat:@"http://daiyancheng.cn/%@",findFrame.findModel.img_path];
    ...
    NSString *shopname=[self subStrFromFitScreen:findFrame.findModel.title];
    ...
    NSString *priceStr=[NSString stringWithFormat:@"%@",findFrame.findModel.store_name];
    self.priceLabel.text=priceStr;
    self.priceLabel.frame=findFrame.priceF;
    //4.设置商品来源的位置
    
    NSString *addressStr=[NSString stringWithFormat:@"%@",findFrame.findModel.add_time_x];
...
-(void)setupAddressLabelWithStr:(NSString*)str
{
    ...
    attach.image = [UIImage imageNamed:@"放一张小图片显示在时间的前面"];
