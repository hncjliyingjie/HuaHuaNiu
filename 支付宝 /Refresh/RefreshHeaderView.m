...
- (void)setScrollView:(UIScrollView *)scrollView
{
    ...
    // 1.设置边框
    self.frame = CGRectMake(0, - RefreshViewHeight, scrollView.frame.size.width, RefreshViewHeight);
...
- (void)setLastUpdateTime:(NSDate *)lastUpdateTime
{
    ...
    // 1.归档
    [[NSUserDefaults standardUserDefaults] setObject:_lastUpdateTime forKey:RefreshHeaderTimeKey];
...
- (void)setState:(RefreshState)state
{
	...
	switch (state) {
        ...
        {
            // 设置文字
            _statusLabel.text = RefreshHeaderReleaseToRefresh;
            // 执行动画
        ...
        {
            // 设置文字
			_statusLabel.text = RefreshHeaderPullToRefresh;
            // 执行动画
            [UIView animateWithDuration:RefreshAnimationDuration animations:^{
        ...
        {
            // 设置文字
            _statusLabel.text = RefreshHeaderRefreshing;
            // 执行动画
            [UIView animateWithDuration:RefreshAnimationDuration animations:^{
                _arrowImage.transform = CGAffineTransformIdentity;
                // 1.增加65的滚动区域
                UIEdgeInsets inset = _scrollView.contentInset;
                inset.top = _scrollViewInitInset.top + RefreshViewHeight;
                _scrollView.contentInset = inset;
                // 2.设置滚动位置
                _scrollView.contentOffset = CGPointMake(0, - _scrollViewInitInset.top - RefreshViewHeight);
            }];
			break;
        }
