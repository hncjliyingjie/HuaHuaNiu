...
{
    // 1.移除以前的监听器
    [_scrollView removeObserver:self forKeyPath:RefreshContentSize context:nil];
    // 2.监听contentSize
    [scrollView addObserver:self forKeyPath:RefreshContentSize options:NSKeyValueObservingOptionNew context:nil];
...
- (void)adjustFrame
{
    ...
    // 设置边框
    self.frame = CGRectMake(0, y, _scrollView.frame.size.width, RefreshViewHeight);
}
...
- (void)setState:(RefreshState)state
{
	...
	switch (state)
    {
        ...
        {
            _statusLabel.text = RefreshFooterReleaseToRefresh;
        ...
        {
            _statusLabel.text = RefreshFooterPullToRefresh;
            
            // 刚刷新完毕
            CGFloat animDuration = RefreshAnimationDuration;
            CGFloat deltaH = [self contentBreakView];
            CGPoint tempOffset;
            
            int currentCount = [self totalDataCountInScrollView];
            if (RefreshStateRefreshing == oldState && deltaH > 0 && currentCount != _lastRefreshCount) {
        ...
        case RefreshStateRefreshing:
        {
            ...
            _statusLabel.text = RefreshFooterRefreshing;
            _arrowImage.transform = CGAffineTransformMakeRotation(M_PI);
            [UIView animateWithDuration:RefreshAnimationDuration animations:^{
                UIEdgeInsets inset = _scrollView.contentInset;
                CGFloat bottom = RefreshViewHeight + _scrollViewInitInset.bottom;
                CGFloat deltaH = [self contentBreakView];
...
{
    [super free];
    [_scrollView removeObserver:self forKeyPath:RefreshContentSize];
}
@end